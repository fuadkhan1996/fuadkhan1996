<div align="center">

# Hi, I'm Fuad Khan 👋

### Lead Software Engineer building reliable SaaS platforms, AI-powered workflows, and cloud-native systems

[![DataCrest](https://img.shields.io/badge/DataCrest-Website-0A66C2?style=for-the-badge&logo=googlechrome&logoColor=white)](https://mydatacrest.com/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)]([https://linkedin.com/in/fuadkhan96](https://www.linkedin.com/in/fuad-khan-fk))
[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:khanfuad1996@gmail.com)

</div>

---

## About Me

I'm a **Lead Software Engineer at [DataCrest](https://mydatacrest.com/)**, focused on designing and delivering production software for complex business workflows.

My work spans backend architecture, cloud infrastructure, frontend development, third-party integrations, workflow automation, and AI-assisted document processing.

- 🏗️ Designing maintainable systems with **Ruby on Rails and PostgreSQL**
- 🤖 Building **AI-powered document extraction and automation workflows**
- ☁️ Developing cloud-native solutions using **AWS**
- 🔌 Integrating complex external platforms and APIs
- ⚡ Building responsive interfaces with **React, Next.js, and TypeScript**
- 👥 Leading development, reviewing architecture, and mentoring engineers
- 🧠 Turning complicated domain requirements into simple product experiences

---

## Engineering Focus

| Area | What I Work With |
|---|---|
| **Backend Architecture** | Ruby on Rails, REST APIs, background processing, service-oriented design |
| **AI and Automation** | LLM workflows, OCR, document extraction, structured data generation |
| **Cloud Infrastructure** | AWS, Lambda, S3, SQS, SES, EC2, Elastic Beanstalk |
| **Frontend Engineering** | React, Next.js, TypeScript, JavaScript |
| **Data and Integrations** | PostgreSQL, third-party APIs, event-driven workflows |
| **Engineering Leadership** | Architecture, code reviews, technical planning, and mentoring |

---

## Technologies

<div align="center">

### Languages and Frameworks

![Ruby](https://img.shields.io/badge/Ruby-CC342D?style=flat-square&logo=ruby&logoColor=white)
![Ruby on Rails](https://img.shields.io/badge/Ruby_on_Rails-D30001?style=flat-square&logo=rubyonrails&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)

### Data, Cloud, and Infrastructure

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonwebservices&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

### AI and Document Processing

![Amazon Bedrock](https://img.shields.io/badge/Amazon_Bedrock-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![LLMs](https://img.shields.io/badge/LLM_Workflows-412991?style=flat-square&logo=openai&logoColor=white)
![OCR](https://img.shields.io/badge/OCR-Document_Processing-5A45FF?style=flat-square)

</div>

---

## What I Enjoy Building

```text
Reliable backend systems
        +
Cloud-native infrastructure
        +
AI-assisted automation
        +
Thoughtful product experiences
```

I particularly enjoy working on:

- Complex business rules and workflow engines
- Intelligent document processing and structured data extraction
- Scalable Ruby on Rails application architecture
- Third-party platform and insurance-system integrations
- Event-driven and asynchronous processing
- Developer-friendly APIs and internal tooling
- Reliable systems for complex operational workflows

---

## GitHub Activity

<div align="center">

  <a href="https://github.com/fuadkhan1996">
    <img
      height="170"
      src="https://github-stats-extended.vercel.app/api?username=fuadkhan1996&show_icons=true&hide_border=true&rank_icon=github&theme=transparent"
      alt="Fuad Khan's GitHub statistics"
    />
  </a>

  <a href="https://github.com/fuadkhan1996">
    <img
      height="170"
      src="https://github-stats-extended.vercel.app/api/top-langs/?username=fuadkhan1996&layout=compact&hide_border=true&theme=transparent&langs_count=6"
      alt="Fuad Khan's most-used languages"
    />
  </a>

</div>

> Most of my recent production work is maintained in private repositories.

---

## Current Interests

- Production-ready AI and LLM systems
- Intelligent document processing
- Event-driven architecture
- Cloud infrastructure and observability
- Software architecture and system design
- Engineering leadership and mentorship
- Improving developer and customer experiences

---

## Professional Approach

```ruby
class EngineeringApproach
  def call
    understand_the_problem
    design_for_maintainability
    build_with_quality
    measure_the_results
    continuously_improve
  end
end
```

I believe strong engineering is not only about writing code. It is about understanding the problem, making thoughtful trade-offs, communicating clearly, and building systems that remain reliable as products and teams grow.

---

## Let's Connect

I'm always interested in discussing software architecture, Ruby on Rails, AI automation, cloud systems, and challenging product problems.

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Fuad_Khan-0A66C2?style=flat-square&logo=linkedin&logoColor=white)]([https://linkedin.com/in/fuadkhan96](https://www.linkedin.com/in/fuad-khan-fk))
[![GitHub](https://img.shields.io/badge/GitHub-fuadkhan1996-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/fuadkhan1996)
[![Email](https://img.shields.io/badge/Email-khanfuad1996%40gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:khanfuad1996@gmail.com)

<br />

**Building reliable systems for complex problems.**

</div>
